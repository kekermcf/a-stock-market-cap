...
/* Watchlist */
.watch-btn{{background:linear-gradient(135deg,#f39c12,#e67e22);color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:14px;cursor:pointer;margin-right:12px;font-weight:600}}
...
.pwd-overlay{{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.6);z-index:2000;justify-content:center;align-items:center}}
...
.watchlist-section{{display:none;background:#fff;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,.06);margin-bottom:24px;overflow:hidden}}
.watchlist-section.active{{display:block}}
.watchlist-section .section-title{{background:linear-gradient(135deg,#f39c12,#e67e22);color:#fff}}
.watchlist-section .section-title .badge{{background:rgba(255,255,255,.3);color:#fff}}
.watchlist-empty{{text-align:center;padding:40px;color:#999;font-size:15px}}
...
<div class="header">
  <div style="display:flex;justify-content:space-between;align-items:flex-start">
    ...
    <button class="watch-btn" onclick="showWatchlist()" title="keke自选股">⭐ 自选</button>
  </div>
</div>
...
<div class="watchlist-section" id="watchlistSection">
  <div class="section-title">
    ...
    <span style="margin-left:auto;font-size:12px;font-weight:400;opacity:.8;cursor:pointer" onclick="document.getElementById('watchlistSection').classList.remove('active')">✕ 关闭</span>
  </div>
  <div class="watchlist-empty" id="wlEmpty">暂无自选股，在股票列表点击 ⭐ 即可添加</div>
...
function updateFavStars() {{
  ...
  // Also update watchlist badge
  document.getElementById('wlBadge').textContent = watchlist.length + ' 只';
}}
...
function checkPwd() {{
  if (document.getElementById('pwdInput').value === WL_PWD) {{
    ...
    renderWatchlist();
    var ws = document.getElementById('watchlistSection');
...
function renderWatchlist() {{
  ...
  for (var i = 0; i < wl.length; i++) {{
    ...
    rows.push('<tr data-code="' + r.ts_code + '" class="' + ahClass.trim() + '">' +
      ...
      '<td>' + ytdHtml(r.ytd_2025) + '</td>' +
      '<td>' + ytdHtml(r.ytd_2026) + '</td>' +
      '<td>' + (r.revenue != null ? numfmt(r.revenue, 1) : '<span class="pending">暂无</span>') + '</td>' +
      '<td class="' + rateCls(r.gpr) + '">' + (r.gpr != null ? Math.round(r.gpr) + '%' : '<span class="pending">-</span>') + '</td>' +
      '<td class="' + (r.net_profit != null ? (r.net_profit >= 0 ? 'fin-pos' : 'fin-neg') : '') + '">' + (r.net_profit != null ? numfmt(r.net_profit, 0) : '<span class="pending">暂无</span>') + '</td>' +
      '<td class="' + rateCls(r.npm) + '">' + (r.npm != null ? Math.round(r.npm) + '%' : '<span class="pending">-</span>') + '</td>' +
        ...
        '<div style="display:flex;justify-content:space-between;align-items:flex-start">' +
          '<div style="flex:1">' +
            ...
            '<div class="mc-info">' +
              ...
              '<span>' + (r.industry || '-') + '</span>' +
            '</div>' +
            '<div class="mc-mv" style="margin-top:6px">' + Math.round(r.total_mv).toLocaleString('zh-CN') + '亿</div>' +
          '</div>' +
...
// Watchlist table click -> open modal
