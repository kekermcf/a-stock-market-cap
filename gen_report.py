...
<div class="container">
  <div class="section">
    ...
    <div class="controls">
      ...
      <select id="hkPremFilter">
        <option value="">全部公司</option>
        <option value="hasPrem">含H股溢价</option>
      </select>
...
function filterData() {{
  ...
  var hkPremVal = document.getElementById('hkPremFilter').value;
  ...
  filtered = allData.filter(function(r) {{
    ...
    if (hkPremVal === 'hasPrem') {{
      var s2 = ahStatus[r.ts_code] || '';
      if (s2 !== 'listed' || r.ah_premium == null) return false;
    }}
    if (hkNewOn) {{
      var s = ahStatus[r.ts_code] || '';
...
document.getElementById('hkPremFilter').addEventListener('change', filterData);
...
document.getElementById('clearFilters').addEventListener('click', function() {{
  ...
  document.getElementById('hkPremFilter').value = '';
  hkNewOn = false;
