...
if os.path.exists(mv_100_200_path):
    ...
    for s in mv_mid:
        ...
        results_all.append({
            ...
            'industry': s.get('industry', ''), 'industry_l1': '', 'industry_l2': '',
            'area': s.get('area', ''), 'close': s.get('close', 0),
            'pct_chg': s.get('pct_chg', ''), 'pe': s.get('pe', ''), 'pb': s.get('pb', ''),
            'total_mv': s.get('total_mv', 0),
            'revenue': None, 'gross_profit': None, 'net_profit': None,
            'gpr': None, 'npm': None,
            'ytd_2024': pct.get('ytd_2024'), 'ytd_2025': pct.get('ytd_2025'),
            'ytd_2026': pct.get('ytd_2026'), 'mv_history': None,
            'list_date': '', 'business_desc': '', 'main_biz': '',
            'products': [], 'regions': [],
        })
